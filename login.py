...
def reset_user_csv():
    ...
    with open('users.csv', 'w', encoding="utf-8") as csv_file:
        ...
        csv_writer.writerow({'name': 'kaf221122', 'password': encrypt_str(decrypt_str('')), 'account type': 'admin'})
        csv_writer.writerow({'name': 'NextToNothing', 'password': encrypt_str(decrypt_str('ËÙÎÌ')), 'account type': 'standard'})
        csv_writer.writerow({'name': 'Custoomer31', 'password': encrypt_str(decrypt_str('ÙÜÚ')), 'account type': 'standard'})
        csv_writer.writerow({'name': 'Seve', 'password': encrypt_str(decrypt_str('ÇÃÄÏÉØËÌÞ')), 'account type': 'standard'})
        csv_writer.writerow({'name': 'Manu', 'password': encrypt_str(decrypt_str('ÙÞÆßÞÐÃ')), 'account type': 'standard'})
        csv_file.close()
